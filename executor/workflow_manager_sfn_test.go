...
func TestUpdateWorkflowStatusJobFailed(t *testing.T) {
	...
	historyEvents := []*sfn.HistoryEvent{
		jobCreatedEvent,
		jobFailedEvent,
	}
	...
	c.mockSFNAPI.EXPECT().
		...
		Do(func(
			...
			cb(&sfn.GetExecutionHistoryOutput{Events: historyEvents}, true)
		})

	c.mockSFNAPI.EXPECT().GetExecutionHistoryWithContext(gomock.Any(), &sfn.GetExecutionHistoryInput{
		ExecutionArn: aws.String(sfnExecutionARN),
		ReverseOrder: aws.Bool(true),
	}).Return(&sfn.GetExecutionHistoryOutput{Events: reversedHistoryEvents(historyEvents)}, nil)
...
func reversedHistoryEvents(events []*sfn.HistoryEvent) []*sfn.HistoryEvent {

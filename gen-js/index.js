...
class WorkflowManager {
  ...
  /**
   * @param {Object} params
   * @param {string} params.definitionName
   * @param params.query
   ...
   * @reject {Error}
   */
  getActiveWorkflows(params, options, cb) {
    return this._hystrixCommand.execute(this._getActiveWorkflows, arguments);
  }
  _getActiveWorkflows(params, options, cb) {
    if (!cb && typeof options === "function") {
      cb = options;
      options = undefined;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const headers = {};
      if (!params.definitionName) {
        rejecter(new Error("definitionName must be non-empty because it's a path parameter"));
        return;
      }
      ...
      if (span) {
        opentracing.inject(span, opentracing.FORMAT_TEXT_MAP, headers);
        span.logEvent("PUT /workflow-definitions/{definitionName}/workflows/active");
        span.setTag("span.kind", "client");
      }

      const requestOptions = {
        method: "PUT",
        uri: this.address + "/workflow-definitions/" + params.definitionName + "/workflows/active",
        json: true,
        timeout,
        headers,
        qs: query,
      ...
      requestOptions.body = params.query;
   ...
   */
  getActiveWorkflowsIter(params, options) {
   ...
   * @reject {Error}
   */
  getInactiveWorkflows(params, options, cb) {
    return this._hystrixCommand.execute(this._getInactiveWorkflows, arguments);
  }
  _getInactiveWorkflows(params, options, cb) {
    if (!cb && typeof options === "function") {
      cb = options;
      options = undefined;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (span) {
        opentracing.inject(span, opentracing.FORMAT_TEXT_MAP, headers);
        span.logEvent("PUT /workflow-definitions/{definitionName}/workflows/inactive");
        span.setTag("span.kind", "client");
      }

      const requestOptions = {
        method: "PUT",
        uri: this.address + "/workflow-definitions/" + params.definitionName + "/workflows/inactive",
        json: true,
        timeout,
        headers,
        qs: query,
   ...
   * @returns {function} iter.forEach - takes in a function, applies it to each resource
   */
  getInactiveWorkflowsIter(params, options) {
    const it = (f, saveResults, cb) => new Promise((resolve, reject) => {
      const rejecter = (err) => {
        reject(err);
        if (cb) {

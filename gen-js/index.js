...
class WorkflowManager {
   ...
   * @reject {Error}
   */
  getStateResourcesByWorkflowDefinition(params, options, cb) {
    return this._hystrixCommand.execute(this._getStateResourcesByWorkflowDefinition, arguments);
  }
  _getStateResourcesByWorkflowDefinition(params, options, cb) {
    if (!cb && typeof options === "function") {
      cb = options;
      options = undefined;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (span) {
        opentracing.inject(span, opentracing.FORMAT_TEXT_MAP, headers);
        span.logEvent("GET /state-resources/{namespace}");
        span.setTag("span.kind", "client");
      }

      const requestOptions = {
        method: "GET",
        uri: this.address + "/state-resources/" + params.namespace + "",
        json: true,
        timeout,
        headers,
        qs: query,
